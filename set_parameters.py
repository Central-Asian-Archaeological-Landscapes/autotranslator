...
class LoadFolders:  # creates a class to load folders within the given path for translation of all files within that folder
    ...
    biglist = [
        archive,
        monument,
    ]  # biglist for the two above lists so they can be cycled through
    l = None
    ...
    def openfolder(self, data_path, filetype):
        ...
        if (
            filetype == "Archive" or filetype == "Monument" or filetype == "Excel"
        ):  # if the file type is Archive or Monument or Excel
                ...
                for data_path, subdirs, files in os.walk(data_path):
                    for s in subdirs:  # for each subdirectory
                        dirpath = os.path.join(
                            data_path, s
                        )  # creates a directory path to add the file name to for a complete file path
                        print(dirpath)
                        if (
                            "archive" in str(dirpath).lower()
                        ):  # if 'ARCHIVE' is in the subdirectory name
                            folder = self.archive
                            self.sortfolder(dirpath, filetype, folder)

                        elif (
                            "monument" in str(dirpath).lower()
                        ):  # otherwise if 'MONUMENT' is in the subdirectory path:
                            folder = self.monument
                            self.sortfolder(dirpath, filetype, folder)
    ...
    def runtransl(
        ...
        input_column,
        start_row,
    ):  # runs the translation methods defined in the translate module
        def show_folders(l, type):
            print("length " + str(len(l)))
            self.l = l
            print(
                "The folder being translated is "
                + str(type)
                + ". The files in this folder are:"
            )  # create label widget saying that and listing files
            for f in l:  # for each entry in l
                n = int(l.index(f)) + 1
                name = str(
                    str(n) + ". " + str(f)
                )  # file name is the path + number beforehand
                print(name)  # creates label so files are listed

        if len(self.archive) == 0 and len(self.monument) == 0 and len(self.docs) == 0:
            print(
                "Empty input! Did you select the wrong filetype or put in an invalid path? Reset and try again."
            )

        if (
            filetype == "Excel" or filetype == "Archive" or filetype == "Monument"
        ):  # if the file type is Excel, Archive or Monument
            ...
            remy = input(
                "If you want to remove files, enter corresponding numbers here separated by comma, otherwise enter N:"
            )
            if remy:
                self.removefiles(
                    remy,
        ...
        if str(remy) == "N":  # if No files are to be removed
            ...
            if "," in str(remy):  # if , is in the string
                ...
                for r in rem:  # for r in the split string
                    del l[
                        int(r) - int(i)
                    ]  # delete the file from list using its index calculated by subtracting i from the number given (because the index will update due to deletion i needs to be updated as well)
                ...
                self.runtransl(
                    ...
                    input_column,
                    start_row,
                )  # runs window with updated files
    ...
    def edit_inputs(
        ...
        sheet_in = input(
            "Enter 'arg' to keep same input, or enter sheet in spreadsheet (ex: 'Data Sheet'):"
        )
        input_col_in = input(
            'Enter "arg" to keep same input or enter columns to be translated separated by space'
        )
        """output_col_in = input('Enter "arg" to keep same input or enter columns into which translated data is put')"""
        start_row_in = input(
            'Enter "arg" to keep same input or enter row number from which to start translation (ex: 5 - exclude column names):'
        )
        ...
        if input_col_in.lower() != "arg":
            input_column = (
                input_col_in.split()
            )  # Assuming input as space-separated values
        ...
        def runn():
            try:
                for file in self.l:
                    ...
                    row_length = pd.read_excel(
                        file, input_sheet, usecols=cols
                    )  # creates panda dataframe to figure out how many rows there are in the sheet
                    ...
                    max_row = (
                        int(len(row_length.dropna())) + 3
                    )  # defines max_row (to be used in ranges as the length of row_length dataframe - as the length of the dataframe with the extracted column)
                    ...
                    # in the first place
                    if int(len(row_length)) < int(
                        start_row
                    ):  # to skip over empty files
                        continue
                ...
                print(
                    ...
                    + str(e.args)
                    + "/n"
                    + str(traceback.print_exc(limit=None, file=None, chain=True))
                )
                print(
                    'If Permission Error check permissions on files and tick "allowed to edit" for everyone or check that files are closed'
                )
