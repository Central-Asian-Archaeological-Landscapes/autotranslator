...
from deep_translator import (
    GoogleTranslator as transl,
)  # uses deep_translator instead of googletrans because
...
class TranslMethods:
    ...
    def load_data(self, filename, input_sheet, input_column, start_row, max_row):
        ...
        skipline = (
            int(start_row) - 1
        )  # this is done b/c pd starts with 0-index so 0 = 1st line, etc. and we want to skip the lines before the start row (so if starting row is 5 we want to skip 4 lines (0,1,2,3)
        data = pd.read_excel(
            filename,
            input_sheet,
            header=None,
            ...
            skiprows=skipline,
            nrows=rows,
        )  # looked at pandas documentation for how to work with excel
        print("Data read from Excel:")
        print(data.head())
        data = data.fillna(
            ""
        )  # fills in the empty cells with empty strings so that they can be processed properly
        ...
        names = pd.read_excel(
            filename, input_sheet, header=1, usecols=col, nrows=1
        )  # creates a separate dataframe for the names of columns (which are not grabbed in the original dataframe due to the starting row being at the data, rather than the columns
        headers = list(
            names.columns
        )  # sets it so can be referenced outside of the function
        transl_columns = []
        ...
        for h in headers:
            ...
            if "name" in str(transl_h).lower():
                ...
                "passport" in str(transl_h).lower()
            ):  # some of the spreadsheets have 'passport of history of USSR' as a column name instead of Description
                transl_h = "Description"
            transl_columns.append(transl_h)
        linked_cols = dict(
            zip(input_column, transl_columns)
        )  # creates a dictionary with the original column names as keys and the translated column names as values
        self.linked_cols = linked_cols
        print(linked_cols)

        untrans_to_dict = data[input_column].to_dict(orient="records")
        untranslated = [
            {linked_cols.get(k, k): v for k, v in entry.items()}
            for entry in untrans_to_dict
        ]
    ...
    def create_glossary(self, ilang, glossfile):
        ...
        if ilang == "ru":
            rus = pd.read_excel(
                glossfile, usecols="B:C"
            )  # uses pandas to read excel file extracting columns B to C (Russian and English)
            rus.to_csv(
                "Rusgloss.csv", index=None, header=True
            )  # converts excel to csv because otherwise there are encoding issues that cannot be addressed in pandas read_excel because it removed ability to specify encoding
            rusdf = pd.DataFrame(
                pd.read_csv("Rusgloss.csv", encoding="utf-8")
            )  # reads created csvfile to dataframe specifying the encoding so the Russian characters are read properly
    ...
    def data_check(self, ilang, olang, filetype, glossfile):
        ...
        keep_capital = [
            ...
            "Primary Name",
            "Primary Address",
        ]  # defines list of columns to keep capitalised - includes 'Main Name', 'Name', 'Author', 'Address' columns which may be in various spreadsheets
        """
        ...
        for col in (
            self.names.columns
        ):  # for each column in the names dataframe created in input_data
            if any(
                c in col.lower() for c in keep_capital
            ):  # if a part of the lowercase column name matches any string in the keep_capital list
                ...
                if un == "" or un is None:
                    ...
                    if ilang == "ru" and olang == "en":
                        for key in gloss:  # for each key in the russian glossary dictionary - so for each Russian term
                            pattern = (
                                "*" + str(key).lower() + "*"
                            )  # the pattern is * wildcard character + the key + * - so it matches wherever the key may be present in the string. Fnmatch is used so * is accepted as a wildcard
                            replacement = str(
                                gloss[key]
                            ).lower()[
                                2:-2
                            ...
                            if fn.fnmatch(un, str(pattern)):
                                ...
                                pats.append(
                                    str(key).lower()
                                )  # and append the Russian word (which has a match) to pats - so the two still correspond with each other by index - lowercased as the rest of the string will be so if it isn't it won't get a match
                                continue
                            else:  # otherwise reset the loop and continue
                                continue

                        for i in range(
                            0, int(int(len(pats)) - 1)
                        ):  # if i = 0 to the length of pats (-1 because len starts from 1 while index starts from 0)
                            un = un.lower().replace(
                                pats[i], reps[i]
                            )  # un becomes the lowercased string, with the pattern replaced
                    ...
                    nu = un.replace(
                        ".,", ","
                    )  # removes duplicated punctuation which can appear after the data check
                    newstring = nu.replace(
                        "..", "."
                    )  # removes duplicated punctuation appearing after data check
                    ...
                    if capitalise:  # if capitalize is set to True from earlier
                        ...
                        nu = newstring.split(
                            "."
                        )  # splits newstring along '.' as separators so it's individual sentences
                        cap = []  # creates empty list to store these in

                        for sent in nu:  # for each sentence in the nu string
                            caps = (
                                sent.strip().capitalize()
                            )  # strips leading/tailing spaces
                            sep = caps.split(
                                " "
                            )  # splits this string along ' ' - so now a consists of all the individual words within sentence z - this is done so initials are capitalised as they end up being lowercase
                            # because the above-splitter is along '.' so things like 'V.Katsev' will become split
                            last = len(
                                str(sep[-1])
                            )  # v is the length of the last word within the list of words in a
                            ...
                            if last == 1:
                                capp = sep[
                                    -1
                                ].capitalize()  # then the last word is capitalised
                                sep.pop(-1)  # the previous last word is removed
                                sep.append(
                                    capp
                                )  # and the new capitalised initial is added instead
                                joined = " ".join(
                                    sep
                                )  # the string is joined back together using ' ' as separator
                                cap.append(joined)  # and appended
                            elif (
                                last > 1
                            ):  # otherwise if the last word is more than one character
                                cap.append(
                                    caps
                                )  # append the capitalised sentence as it was

                        fin = ". ".join(
                            cap
                        )  # rejoins the sentences in cap with a period
            ...
            for data_dict in untranslated:
                ...
                for col, item in data_dict.items():  # for strings in untranslated
                    if col.lower() == "caal_id":
                        transl_data[col] = (
                            item  # we don't want to translate the CAAL ID as it's already in English
                        )
                        # repeat for any other columns that should not be translated
                    else:
                        transl_text = str(
                            item
                        )  # converts it into a string (to be sure that it is actually coming out as string value)
                        transltxt = transl(
                            source=ilanguage, target=olanguage
                        ).translate(
                            transl_text
                        )  # translates the string - taken from deep_translator documentation#
    ...
    def combinedata(self):
        ...
        for data_dict in self.translated:
            ...
            for (
                col,
                item,
            ) in data_dict.items():  # for each column and item in the translated data
                ...
                if col in combine:  # if the column is in the combine list
                    ...
                    combined = (
                        str(original) + " / " + str(item)
                    )  # combines untranslated and translated data together

                    combined_data[col] = (
                        combined  # adds the combined string back to dictionary with the same column name
                    )
            ...
            final_data = {
                ...
                + " / "
                + str(combined_data["Name"]),
                "Description": combined_data["Description"],
                "Type": str(combined_data["Type of material"])
                + ". "
                + str(combined_data["Quantity"]),
            }
    ...
    def input_data(self, filename, input_sheet, input_column, start_row, max_row):
        ...
        i = 0
        for row in range(
            int(start_row), int(len(self.translated)) + 1
        ):  # for row in the range of starting row to max_row
            cell = str(
                input_column + str(row)
    ...
    def dict_to_csv(self, filename):
        csv_file = (
            str(Path(filename).stem) + "_translated_data.csv"
    ...
    def reset(
        self,
    ):  # resets the function so there are no leftover values in the lists for the future values
...
class TranslateRun:  # the class to run the above methods
    ...
    def runn(
        ...
        filetype,
        glossfile,
    ):  # the whole translation and SQL input process with the associated variables
    ...
    def runtransl(
        ...
        print(
            "Note: If file contains forms/macros you will need to manually close/log into these when they pop-up during entry"
        )
